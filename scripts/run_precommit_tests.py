...
def run_pytest_tests() -> Tuple[bool, str]:
    ...
    print("⚠️  All pytest attempts failed with IO buffer issues, falling back to direct verification")
...
def run_direct_verification() -> Tuple[bool, str]:
    ...
    try:
        ...
        tests = [
            ...
            # Python logic
            lambda: chat_repl._should_use_temp_file('print("hello")', 'python') == False,
            lambda: chat_repl._should_use_temp_file('def test():\n    pass', 'python') == True,
            
            # JavaScript logic
            lambda: chat_repl._should_use_temp_file('console.log("hello")', 'javascript') == False,
            lambda: chat_repl._should_use_temp_file('function test() {}', 'javascript') == True,
            
            # PowerShell logic (keywords)
            lambda: chat_repl._should_use_temp_file('Get-Date', 'powershell') == True,
            lambda: chat_repl._should_use_temp_file('Write-Host "hello"', 'powershell') == False,
            
            # Batch logic (keywords)
            lambda: chat_repl._should_use_temp_file('set NAME=John', 'bat') == True,
            lambda: chat_repl._should_use_temp_file('echo hello', 'bat') == False,
            
            # Edge cases
            lambda: chat_repl._should_use_temp_file('', 'python') == False,
            lambda: chat_repl._should_use_temp_file('x' * 250, 'python') == True,
        ]
...
def check_code_quality() -> Tuple[bool, str]:
    ...
    exit_code, stdout, stderr = run_command([
        "python", "-m", "py_compile", "xandai/__init__.py"
    ])
    if exit_code == 0:
