...
class TestBatchExecution:
    ...
    def test_simple_batch_should_use_inline(self, chat_repl):
        ...
        complex_codes = [
            'cd C:\\'  # Contains backslash, triggers temp file
        ]
...
if %errorlevel% == 0 (
    ...
    echo No internet connection
        ...
        for code in complex_codes:
            should_use_temp = chat_repl._should_use_temp_file(code, 'bat')
            assert should_use_temp == True, f"Complex Batch code should use temp file: {code[:50]}..."
    ...
    @patch('os.unlink')
    def test_complex_batch_temp_file_execution(self, mock_unlink, mock_temp, mock_execute, chat_repl):
...
pause'''
    ...
    def test_batch_error_handling(self, mock_execute, chat_repl):
        ...
        error_calls = [call for call in mock_console.call_args_list 
                      if 'Error executing bat code' in str(call)]
    ...
    def test_batch_variable_detection(self, chat_repl):
        ...
        for code in inline_commands:
            should_use_temp = chat_repl._should_use_temp_file(code, 'bat')
            assert should_use_temp == False, f"Simple batch variable usage should use inline: {code}"
...
goto loop''',
    ...
    echo Unknown option
        ...
        for code in control_flow_codes:
            should_use_temp = chat_repl._should_use_temp_file(code, 'bat')
            assert should_use_temp == True, f"Batch control flow should use temp file: {code[:30]}..."
