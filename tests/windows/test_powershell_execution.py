...
class TestPowerShellExecution:
    ...
    def test_simple_powershell_should_use_inline(self, chat_repl):
        ...
        for code in keyword_codes:
            should_use_temp = chat_repl._should_use_temp_file(code, 'powershell')
            assert should_use_temp == True, f"PowerShell code with keywords should use temp file: {code}"
...
while ($counter -lt 3) {
    ...
    $counter++
        ...
        for code in complex_codes:
            should_use_temp = chat_repl._should_use_temp_file(code, 'powershell')
            assert should_use_temp == True, f"Complex PowerShell code should use temp file: {code[:50]}..."
    ...
    @patch('os.unlink')
    def test_complex_powershell_temp_file_execution(self, mock_unlink, mock_temp, mock_execute, chat_repl):
...
Write-Host (Get-Greeting -Name "World")'''
        ...
        mock_console.assert_any_call("[dim]Creating temporary powershell file for execution...[/dim]")
    ...
    def test_powershell_cmdlet_detection(self, chat_repl):
        ...
        cmdlet_tests = [
            ...
            ('New-Object System.Collections.ArrayList', False),  # No keywords matched, long but <200 chars
...
Format-Table Name, Status, StartType -AutoSize''',
    ...
    $_ * 2
        ...
        for code in pipeline_codes:
            should_use_temp = chat_repl._should_use_temp_file(code, 'powershell')
            assert should_use_temp == True, f"PowerShell pipeline should use temp file: {code[:30]}..."
